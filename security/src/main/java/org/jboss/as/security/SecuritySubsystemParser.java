...
public class SecuritySubsystemParser implements XMLStreamConstants, XMLElementReader<List<ModelNode>>,
        ...
        for (String key : keys) {
            ...
            switch (element) {
                ...
                case CERTIFICATE_MAPPING: {
                    writeCertMapping(writer, policyDetails.get(CERT_MAPPING, CLASSIC));
                    break;
                }
            }
        }
    ...
    private void writeCertMapping(XMLExtendedStreamWriter writer, ModelNode modelNode) throws XMLStreamException {
        if (modelNode.isDefined() && modelNode.asInt() > 0) {
            writer.writeStartElement(Element.CERTIFICATE_MAPPING.getLocalName());
            CertMappingResourceDefinition.CODE.marshallAsAttribute(modelNode, false, writer);
            writer.writeEndElement();
        }
    }
    ...
    private void parseSecurityDomain(List<ModelNode> list, XMLExtendedStreamReader reader, ModelNode parentAddress) throws XMLStreamException {
        ...
        while (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            ...
            switch (element) {
                ...
                case CERTIFICATE_MAPPING: {
                    parseCertMapping(list, address, reader);
                    break;
                }
                default: {
                    throw unexpectedElement(reader);
    ...
    private void parseCertMapping(List<ModelNode> list, ModelNode parentAddress, XMLExtendedStreamReader reader) throws XMLStreamException {
        ModelNode op = appendAddOperation(list, parentAddress, CERT_MAPPING, CLASSIC);
        EnumSet<Attribute> required = EnumSet.of(Attribute.CODE);
        final int count = reader.getAttributeCount();
        for (int i = 0; i < count; i++) {
            requireNoNamespaceAttribute(reader, i);
            ...
            switch (attribute) {
                case CODE: {
                    CertMappingResourceDefinition.CODE.parseAndSetParameter(value, op, reader);
                    break;
                }
                default:
                    throw unexpectedAttribute(reader, i);
