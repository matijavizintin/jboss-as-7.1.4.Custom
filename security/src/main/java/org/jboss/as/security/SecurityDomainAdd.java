...
import static org.jboss.as.security.Constants.CERT_MAPPING;
...
import org.jboss.security.JBossCertificateMappingSecurityManager;
...
class SecurityDomainAdd extends AbstractAddStepHandler {
    ...
    public void launchServices(OperationContext context, String securityDomain, ModelNode model, ServiceVerificationHandler verificationHandler, List<ServiceController<?>> newControllers) throws OperationFailedException {
        ...
        final JBossCertificateMappingSecurityManager certificateMappingSecurityManager = createCertificateMappingSecurityManager(securityDomain, model);
        ...
        final SecurityDomainService securityDomainService = new SecurityDomainService(securityDomain,
                applicationPolicy, jsseSecurityDomain, certificateMappingSecurityManager, cacheType);
    ...
    private JBossCertificateMappingSecurityManager createCertificateMappingSecurityManager(String securityDomain, ModelNode node) {
        node = peek(node, CERT_MAPPING, CLASSIC);
        if (node == null) {
            return null;
        }

        final JBossCertificateMappingSecurityManager manager = new JBossCertificateMappingSecurityManager(securityDomain);

        String codeName = extractCode(node, ModulesMap.CERT_MAPPING_MAP);
        manager.setMappingClassName(codeName);
